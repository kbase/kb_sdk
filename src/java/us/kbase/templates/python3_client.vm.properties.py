...
#set( $comma_args = "#if($method.arg_count>0), ${method.args}#{else}#{end}" )
    ...
    def ${method.name}_async(self${comma_args}, json_rpc_context = None):
        ...
        return self._call('${module.module_name}.${method.name}_async',
                          [${method.args}], json_rpc_context)[0]

    def ${method.name}_check(self, job_id):
        resp = self._call('${module.module_name}.${method.name}_check', [job_id])
        return resp[0]

    def ${method.name}(self${comma_args}, json_rpc_context = None):
        job_id = self.${method.name}_async(${method.args}#if($method.arg_count>0), #{end}json_rpc_context)
        while True:
            time.sleep(self.async_job_check_time)
            job_state = self.${method.name}_check(job_id)
