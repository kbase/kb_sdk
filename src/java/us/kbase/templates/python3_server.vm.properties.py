...
class Application(object):
    ...
    def __init__(self):
        submod = get_service_name() or '${service_name}'
        ...
        # set( $spectype = $p.baretype )
        # set( $type = $specToPy.get($spectype) )
        # set( $void = $typelist.add($type) )
        # end
        self.rpc_service.add(impl_${module.module_name}.${method.name},
                                                         name = '${module.module_name}.${method.name}',
                                                                types = [$display.join(${typelist}, ", ")])
        # if( $authenticated )
        self.method_authentication['${module.module_name}.${method.name}'] = '${method.authentication}'
        # end
        # end
        # if( !$status_in_kidl )
        self.rpc_service.add(impl_${module.module_name}.status,
                                   name = '${module.module_name}.status',
                                          types = [dict])
        # end
        # end
        # if( $authenticated )
...
try:
    ...
    if config is not None and config.get('gevent_monkeypatch_all', False):
        print
        "Monkeypatching std libraries for async"
